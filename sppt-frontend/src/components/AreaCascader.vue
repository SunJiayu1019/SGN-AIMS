...
// 根据外部传入的值（id 或 code），反推 省/市/区 三级选中（编辑回显时用）
function syncFromModel(val) {
  ...
  // 自底向上回填
  provinceId.value = ALL; cityId.value = ALL; districtId.value = ALL
  if (node.level === 1) {
    ...
    cityId.value = city ? city.id : ALL
    provinceId.value = city ? city.parentId : ALL
    districtId.value = node.id
  }
}
